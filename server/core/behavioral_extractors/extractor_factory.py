...
class BehavioralExtractorFactory:
    ...
    @staticmethod
    def create_extractor(extractor_type: str = "real", config: Dict[str, Any] = None) -> BaseBehavioralExtractor:
        """
        Create a behavioral extractor based on type and configuration
        
        Args:
            extractor_type: Type of extractor ("real", "custom")
        ...
        if extractor_type == "real":
            ...
            logger.warning("Custom extractor type not implemented yet, falling back to real")
            api_endpoint = config.get("api_endpoint")
            api_key = config.get("api_key")
            extractor = RealBehavioralExtractor(api_endpoint=api_endpoint, api_key=api_key)
            
        else:
            logger.warning(f"Unknown extractor type: {extractor_type}, falling back to real")
            api_endpoint = config.get("api_endpoint")
            api_key = config.get("api_key")
            extractor = RealBehavioralExtractor(api_endpoint=api_endpoint, api_key=api_key)
